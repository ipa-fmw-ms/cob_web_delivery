...
from Queue import PriorityQueue #todo: customize
import threading
...
class QueServer:
    def __init__(self):
        ...
        self.t = threading.Thread(target=self.send_que)
        self.t.start()
    ...
    def handle_order_que(self, req):
        ...
        self.pq.put((req.priority, goal)) #append goal
    ...
    def send_que(self):
        print "send que thread started"
        while True:
            try:
                goal = self.pq.get()
                print goal[1].item
                self.ac.send_goal(goal[1])
